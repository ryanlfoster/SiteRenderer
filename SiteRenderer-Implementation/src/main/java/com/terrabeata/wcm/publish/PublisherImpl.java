...
import com.terrabeata.wcm.publish.api.ResourceRenderer;
import com.terrabeata.wcm.publish.api.Transport;
import com.terrabeata.wcm.publish.internal.FTPTransport;
import com.terrabeata.wcm.publish.internal.FileTransport;
import com.terrabeata.wcm.publish.internal.WebDAVTransport;
...
public class PublisherImpl extends SlingAdaptable implements Publisher {
	...
	static final String NAME = PublishConstants.PROPERTY_NAME;
	
	@Property(description="%publisher.host.description", label="Remote Host")
	static final String HOST = PublishConstants.PROPERTY_HOST;
	
	@Property(description="%publisher.port.description", label="Port")
	static final String PORT = PublishConstants.PROPERTY_PORT;
	...
	static final String PROTOCOL = PublishConstants.PROPERTY_PROTOCOL;
	
	@Property(description="%publisher.root.directory.description", label="Root Directory")
	static final String ROOT_DIRECTORY = PublishConstants.PROPERTY_ROOT_DIRECTORY;
	
	@Property(description="%publisher.url.description", label="Final URL")
	static final String URL = PublishConstants.PROPERTY_URL;
	
	@Property(description="%publisher.category.description", label="Category")
	static final String CATEGORY = PublishConstants.PROPERTY_CATEGORY;
	
	@Property(description="%publisher.credentials.username.description", label="User name")
	static final String USER_NAME = PublishConstants.PROPERTY_USER_NAME;

	@Property(description="%publisher.credentials.password.description", label="User password")
	static final String PASSWORD = PublishConstants.PROPERTY_PASSWORD;
	...
	private void update(ComponentContext context) {
		...
		switch (getProtocol())
		{
		case PublishConstants.PROTOCOL_WEBDAV :
			transport = new WebDAVTransport(this);
			break;
		case PublishConstants.PROTOCOL_FTP :
		case PublishConstants.PROTOCOL_SFTP :
			transport = new FTPTransport(this);
			break;
		case PublishConstants.PROTOCOL_FILE :
		default :
	...
	public boolean process(Event job) {
		...
		String action =
				OsgiUtil.toString(job.getProperty(PublishConstants.PROPERTY_EVENT_ACTION), "");
		
		log.debug("process:: action={}", action);
		
		if (action == PublishConstants.ACTION_FILE_ADD) {
			...
			log.debug("[1]");
			String resourcePath =
					OsgiUtil.toString(job.getProperty(PublishConstants.PROPERTY_EVENT_RESOURCE_PATH), "");
			String destinationPath =
					OsgiUtil.toString(job.getProperty(PublishConstants.PROPERTY_EVENT_DESTINATION_PATH), "");
			String fileName =
					OsgiUtil.toString(job.getProperty(PublishConstants.PROPERTY_EVENT_FILE_NAME), "");
	...
	public int getProtocol() {
		return OsgiUtil.toInteger(properties.get(PROTOCOL), PublishConstants.PROTOCOL_FILE);
	}
